...
- 钥匙串里有 **Developer ID Application** 证书（本项目用 `lei hua (67KN33LJAQ)`）。

### 一条命令：导出 + 签名
```bash
bash packaging/build_macos.sh
```
产出 `build/macos/VoxelCraft-macos.zip`：已用 Developer ID 签名、启用硬化运行时、带安全时间戳。
...
```bash
NOTARY_PROFILE=voxelcraft-notary bash packaging/build_macos.sh
```
完成后 `spctl -a -t exec -vvv build/macos/stage/VoxelCraft.app` 应显示 `accepted / source=Notarized Developer ID`。
