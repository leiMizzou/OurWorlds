...
func _initialize() -> void:
	...
	# RailBridgeNet：铁轨从 cell4(-1,-1)→cell5(169,-1) 经过 chunk(-1,-1)
	var rc := Chunk.new(-1, -1)
	...
	# 铁轨在 chunk(-1,-1) 而非 chunk(0,0)
	var ig_rail := Chunk.new(-1, -1)
	gen.generate(ig_rail)
	check(has_block_type(ig_rail, BlockLibrary.RAIL), "generate() stamps rail at chunk(-1,-1)")
