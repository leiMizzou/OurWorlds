...
日期：2026-06-06
状态：已实现并入 main；本文保留为设计记录

## 背景与目标

当时背景：OurWorlds 是单机 + 本地 agent 桥（`scripts/AgentBridge.gd`，`127.0.0.1`，单客户端）。本功能目标是在**单机一局内**提供：
...
- 规模：本地单局，**多 agent 同时在线**（AgentBridge 已从单客户端演进为多客户端）。
